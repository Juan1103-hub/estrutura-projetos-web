...
// Acessível para role 'farmacia' e role 'qualidade' (Q-001)
...
export default function IndicadoresFarmaciaPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    const result = await fetchIndicatorsWithTarget("farmacia")
    setData(result)
    setLoading(false)
  }, [])
  ...
  // Preparar dados dos gráficos quando os indicadores mudam
  useEffect(() => {
    if (!data) return

    // Gráficos de linha — um por indicador
    Promise.all(
    ...
    // Gráfico de barra — último mês disponível
